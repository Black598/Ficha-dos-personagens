...
export function SoulGrimoire({ souls, updateSouls }) {
    ...
    return el('div', { key: 'soul-grimoire-root', className: "bg-slate-900/50 border-2 border-red-900/30 rounded-[3rem] p-8 shadow-2xl space-y-8" }, [
        // Header
        el('div', { key: 'header', className: "text-center space-y-2 border-b border-red-900/20 pb-6" }, [
        ...
        el('div', { key: 'main-grid', className: "grid grid-cols-1 lg:grid-cols-2 gap-8" }, [
            ...
            // Tabela de Debuffs
            el('div', { key: 'debuffs-container', className: "bg-black/20 p-6 rounded-3xl border border-red-900/10 shadow-inner overflow-hidden" }, [
                el('h3', { key: 'debuffs-title', className: "text-[10px] font-black text-red-500 uppercase tracking-widest mb-4" }, "O Preço do Além"),
                el('div', { key: 'debuffs-list', className: "space-y-4 max-h-[250px] overflow-y-auto pr-2 custom-scrollbar" }, DEBUFFS.map((d, i) => (
                    el('div', { key: `debuff-${i}`, className: "group border-b border-white/5 pb-2 last:border-0" }, [
                        el('div', { key: 'header', className: "flex justify-between items-center mb-1" }, [
                            el('span', { key: 'title', className: `text-[11px] font-black ${d.color}` }, `${d.deaths} Mortes: ${d.title}`),
                        ]),
